...
class TestCounter(unittest.TestCase):
    ...
    def test_difference_if_arrival_before_exit_then_timedelta(self):
        ...
        event2 = Event(datetime(2012,1,5,14,15,40), EventTypes.LEAVE)
    ...
    def test_difference_if_exit_before_arrival_then_0(self):
        event1 = Event(datetime(2012,1,5,12,30,25), EventTypes.LEAVE)
    ...
    def test_difference_if_not_good_event_type_order_raise_exception(self):
        evt1 = Event(datetime(2012,1,5,12,30,25), EventTypes.LEAVE)
        evt2 = Event(datetime(2012,1,5,14,15,40), EventTypes.LEAVE)
    ...
    def test_difference_events_overlap_raise_exception(self):
        evt1 = Event(datetime(2012,1,5,14,15,40), EventTypes.LEAVE)
    ...
    def test_count_events_list_ends_with_exit(self):
        events = [Event(datetime(2012,1,5,7,15,25), EventTypes.ARRIVAL),
                  Event(datetime(2012,1,5,11,30,0), EventTypes.LEAVE),
                  Event(datetime(2012,1,5,12,5,30), EventTypes.ARRIVAL),
                  Event(datetime(2012,1,5,15,50,55), EventTypes.LEAVE)]
        delta = Counter.count_event_list(events)
        self.assertEqual(delta, timedelta(0, 0, 0, 0, 0, 8), "The difference between ARRIVAL and LEAVE should be correct")
    ...
    def test_count_events_list_arrival_first(self):
        with self.assertRaises(InvalidEventTypeOrderException):
            Counter.count_event_list([Event(datetime(2012,1,5,11,30,0), EventTypes.LEAVE),
